...
class TestMainMock:
    def test_cli_main_mock_401(self, monkeypatch):
        ...
        with patch("delete_workflow_runs.cli.get_auth") as mock_auth:
            ...
            result = runner.invoke(main, ["--repo-url", "https://github.com/owner/repo", "--max-days", "30"])
            # print(f'\nMain result: {result}')
            # print(result.stdout)
            # print(result.stderr)
    ...
    def test_cli_main_mock_404(self):
        ...
        with patch("delete_workflow_runs.cli.get_auth") as mock_auth:
            ...
            result = runner.invoke(
                main, ["--repo-url", "https://github.com/invalid/repo", "--max-days", "30", "--dry-run", "true"]
            )
            assert result.exit_code == 1
    ...
    def test_cli_main_mock_non_github_url(self):
        ...
        with patch("delete_workflow_runs.cli.get_auth") as mock_auth:
            ...
            mock_auth.return_value = mock_gh
            result = runner.invoke(
                main, ["--repo-url", "https://github-test.com/owner/repo", "--max-days", "30", "--dry-run", "true"]
            )
            assert result.exit_code == 1
...
class TestMain:
    def test_cli_main_input_false(self):
        ...
        runner = CliRunner()
        result = runner.invoke(
            main, ["--repo-url", "https://github.com/tagdots-dev/workflow-test", "--max-days", "NA", "--dry-run", "false"]
        )
        # print(f'\nMain result: {result}')
        # print(result.stdout)
        # print(result.stderr)
    ...
    def test_cli_main_min_runs_dry_run(self):
        ...
        runner = CliRunner()
        result = runner.invoke(
            main, ["--repo-url", "https://github.com/tagdots-dev/workflow-test", "--min-runs", "100", "--dry-run", "true"]
        )
    ...
    def test_cli_main_max_days_dry_run(self):
        ...
        runner = CliRunner()
        result = runner.invoke(
            main, ["--repo-url", "https://github.com/tagdots-dev/workflow-test", "--max-days", "5", "--dry-run", "true"]
        )
        assert result.exit_code == 0
        assert "dry-run: True" in result.output
    ...
    def test_cli_main_min_runs_dry_run_404(self):
        ...
        runner = CliRunner()
        result = runner.invoke(
            main, ["--repo-url", "https://github.com/tagdots-dev/repo-not-found", "--min-runs", "100", "--dry-run", "true"]
        )
        assert result.exit_code > 0
        assert "dry-run: True" in result.output
